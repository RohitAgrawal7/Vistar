...
function throwIfError(error: { message: string; code?: string } | null, fallback: string) {
  ...
  if (blocked) {
    throw new ApiError(
      "Supabase blocked this write. In the SQL editor, run backend/supabase/schema.sql (includes policies).",
      403,
    );
  }
...
async function load(): Promise<FloorState> {
  if (!isSupabaseConfigured()) {
    throw new ApiError(
      "Supabase is not configured. Set SUPABASE_URL and SUPABASE_PUBLISHABLE_KEY (or SUPABASE_SERVICE_ROLE_KEY) in backend/.env.local",
      503,
    );
  }
